...
@override
  void initState() {
    Get.find<PopularController>().getPopularItem();
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return Container(
      height: 200,
      child:GetBuilder<PopularController>(builder: (popular){
        return ListView.builder(
            itemCount: popular.popularItem.length,
        scrollDirection: Axis.horizontal,
        itemBuilder: (context,index){
        return GestureDetector(
          onTap: ()async{
            Navigator.push(context, MaterialPageRoute(builder: (context){
              return PopularDetail(popularData: popular.popularItem[index],);
            }));
          },
          child: Container(
              ...
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    image: DecorationImage(image: NetworkImage(popular.popularItem[index].image.toString()),
                    fit: BoxFit.cover
                    )
                  ),
                ),
                SizedBox(height: 10,),
                Text(popular.popularItem[index].title.toString(),style: TextStyle(
                  fontSize: 15,
